...
static void WriteProcessPipe(Service::Interface* self) {
    ...
    u32 size     = cmd_buff[2];
    u32 new_size = cmd_buff[3]; // IPC Description of a static buffer
    u32 buffer   = cmd_buff[4];

    if (IPC::StaticBufferDesc(size, 1) != new_size) {
        LOG_ERROR(Service_DSP, "Static Buffer desc failed validation. number=%u, size=0x%X, new_size=0x%X, buffer=0x%08X",
    ...
    DSP::HLE::PipeWrite(number, message);
